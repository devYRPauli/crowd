...
describe('Simulation', () => {
  ...
  it('reaches and leaves a seat in a row from the end of the row', () => {
    ...
    // conference hall stood among the seated delegates until they gave up.
    // With the door behind the rows the short way in is over the backs, and
    // with it in front the short way out.
    const walkingState = agentStateIndex('walking')
    const seatedState = agentStateIndex('seated')

    const over = { north: { entering: 0, leaving: 0 }, south: { entering: 0, leaving: 0 } }
    for (const door of ['north', 'south'] as const) {
      const b = new PlanBuilder()
      const room = b.room(0, 0, 10, 8)
      b.door(room[door], 5, DEFAULT_DOUBLE_DOOR_WIDTH, 'door', 'both')
      b.seatingBlock(5, 3, 4, 3.3)
      const seating = b.zone('seating', 3.2, 2.4, 6.8, 6.4, 'Rows')
      const entry =
        door === 'north'
          ? b.zone('entry', 4, 6.7, 6, 7.7, 'In')
          : b.zone('entry', 4, 0.3, 6, 1.3, 'In')
      const plan = b.build()
          ...
          for (let i = 0; i < count; i++) {
            ...
            rows.forEach((row, r) => {
              ...
              if (before !== undefined && before !== now) {
                if (sat.has(id)) over[door].leaving++
                else over[door].entering++
              }
              side.set(key, now)
            })
          }
    ...
    // with the door behind, 2 of them on the way in. Then 5 were left with the
    // door behind and 2 with it in front, all leaving: pushed off the passage,
    // they took the way to the door over a row, or saw the door over one and
    // walked straight at it.
    expect(over.north).toEqual({ entering: 0, leaving: 0 })
    // Not clean yet: somebody pushed out of their passage on the way in is
    // sent round again from the nearer end, and the way there can run over the
    // back of their own row. Measured 2 (1, 1, 0, 0); the target is 0.
    expect(over.south.leaving).toBe(0)
    expect(over.south.entering).toBeLessThanOrEqual(2)
  }, 120_000)
